...
#W_z1 = tf.Variable(xavier_init([totalModel,128]))
#b_z1 = tf.Variable(tf.zeros(shape=[128]))
#W_z2 = tf.Variable(xavier_init([128,z_dim]))
#b_z2 = tf.Variable(tf.zeros(shape=[z_dim]))
...
def EncodeNet(k):
	#h_z1 = tf.nn.relu(tf.matmul(k, W_z1) + b_z1)
	#z_digit = tf.matmul(h_z1, W_z2) + b_z2
