...
def ProbNetSamp(totalModel, c_dim, label_list, samp=500000, prob=0.94):
	x_re = uniform_samp(samp,3)
	kid = random.randint(0, totalModel-1)
	label = label_list[kid]
	print("Samp id: " + str(kid+1))
	k_re = KeySamp(samp, totalModel, kid)
	c_re = CondTensor(label, c_dim, samp)
	y_re = sess.run(y_reconst, feed_dict={x_: x_re, k_: k_re, c_: c_re})
	pc_re = []
...
def CondTensor(label, c_dim, mb_size):
	c = [0]*c_dim
	c[label] = 1.0
	c_list = []
...
W1 = tf.Variable(xavier_init([3+z_dim+c_dim,512]))
...
def ProbNet(x, z, c):
	x_z = tf.concat(axis=1, values=[x, z, c])
...
y_reconst = ProbNet(x_, z_encode, c_)
y_sample = ProbNet(x_, z_, c_)
...
path = "3d_model/ModelNet10_5class/"
pc_list = []
tree_list = []
label_list = []

for i in range(c_dim):
	[pc, tree] = LoadDataId(path + str(i) + "/", classModel)
	pc_list.extend(pc)
	tree_list.extend(tree)
	label_list.extend([i]*len(pc))

for i in range(100000):
	kid = random.randint(0,totalModel-1)
	label = label_list[kid]
	c_list = CondTensor(label, c_dim, int(mb_size*3/2))
	...
	c_samp = np.asarray(c_list)

	_, _loss = sess.run([solver, loss],  feed_dict={x_: x_samp, k_: k_samp, y_: y_samp, c_: c_samp})

	if i%1000 == 0:
		print(str(i) + " " + str(_loss))
		if i%10000 == 0:
			pc_re = ProbNetSamp(totalModel, c_dim, label_list, samp=50000, prob=0.9)
			DrawPc(pc_re, show=False, filename="out/" + str(i))


print("Save parameter ...")
saver = tf.train.Saver()
save_path = saver.save(sess, "tf_save/chair_glo_cond.ckpt")